...
internal sealed class WorkItemGroup : IThreadPoolWorkItem, IWorkItemScheduler
{
    ...
    #region IThreadPoolWorkItem implementations
    ...
    #region IWorkItemScheduler implementations
    ...
    public string DumpStatus()
    {
        lock (_lockObj)
        {
            ...
            sb.Append(this);
            sb.Append(
                $". Currently QueuedWorkItems={_workItems.Count}; Total Enqueued={_totalItemsEnqueued}; Total processed={_totalItemsProcessed}; ");
            if (_currentTask is Task task)
            {
                sb.Append(
                    $" Executing Task Id={task.Id} Status={task.Status} for {TimeSpan.FromMilliseconds(Environment.TickCount64 - _currentTaskStarted)}.");
            }

            sb.Append($"TaskRunner={TaskScheduler}; ");
            if (GrainContext != null)
            {
                ...
                sb.Append($"Detailed context=<{detailedStatus}>");
            }
            return sb.ToString();
        }
    }
    ...
    public override string ToString() =>
                               $"{(GrainContext is SystemTarget ? "System*" : string.Empty)}WorkItemGroup:Name={GrainContext?.ToString() ?? "Unknown"},WorkGroupStatus={_state}";
